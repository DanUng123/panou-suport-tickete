 ...
 */
async function findByClientReference(clientReference, daysBack = 5) {
  const now = new Date();
  ...
  const match = (data.PrintDataInfoList || []).find((p) => p.ClientReference === clientReference);
  return match || null;
}
...
async function createParcel(order) {
  ...
  if (data.PrintLabelsErrorList && data.PrintLabelsErrorList.length) {
    ...
    if (err.ErrorCode === 18) {
      const existing = await findByClientReference(String(order.mpId));
      if (existing) {
        return {
