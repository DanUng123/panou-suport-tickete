...
function cfg(company) {
  return {
    ...
    typeOfPrinter: company.glsTypeOfPrinter || process.env.GLS_TYPE_OF_PRINTER || 'Connect',
...
/**
 * Creeaza un AWB de tip Exchange (Colet la Schimb) -- serviciul GLS "XS",
 * CONFIRMAT LIVE cu contul real al utilizatorului. O singura cerere
 * produce doua colete intr-un singur raspuns: primul (index 0) e ridicarea
 * de la client (Tur), al doilea (index 1) e livrarea coletului de schimb
 * catre client (Retur) -- ambele in acelasi PDF.
 *
 * NOTA: ordinea exacta index 0=Tur/index 1=Retur e o presupunere
 * rezonabila (Tur=PickupAddress mentionata prima, Retur=DeliveryAddress
 * mentionata a doua), NECONFIRMATA explicit -- daca dupa folosire reiese
 * ca sunt inversate, se schimba usor mai jos.
 */
async function createExchangeAwb(company, pickup) {
  const c = cfg(company);
  ...
  const parcel = {
    ...
    Count: 1,
    Content: `Colet la schimb — ${pickup.ticketId}`,
  ...
  if (data.PrintLabelsErrorList && data.PrintLabelsErrorList.length) {
    const err = data.PrintLabelsErrorList[0];
    throw new Error(`GLS a refuzat cererea Exchange (cod ${err.ErrorCode}): ${err.ErrorDescription}`);
  }
  if (!data.PrintLabelsInfoList || data.PrintLabelsInfoList.length < 2) {
    throw new Error('GLS nu a întors ambele colete așteptate pentru serviciul Exchange. Răspuns neașteptat.');
  }
  if (!data.Labels) {
    throw new Error('GLS a generat coletele, dar nu a întors eticheta PDF.');
  }
  const [pickupInfo, deliveryInfo] = data.PrintLabelsInfoList;
  return {
    trackingNumber: String(pickupInfo.ParcelNumber),
    parcelId: pickupInfo.ParcelId,
    secondaryAwbNumber: String(deliveryInfo.ParcelNumber),
    secondaryParcelId: deliveryInfo.ParcelId,
    labelPdf: Buffer.from(data.Labels, 'base64'),
  };
}
...
/**
 * Creeaza AWB-ul secundar (retur), pentru Colet la Schimb -- livrare de la
 * NOI catre client (directie inversa fata de AWB-ul de ridicare). GLS nu are
 * un serviciu SWAP nativ, ca Sameday (care intoarce ambele AWB-uri dintr-un
 * singur apel) -- de aceea facem aici un al doilea apel separat, distinct.
 */
async function createExchangeReturnAwb(company, pickup) {
  const c = cfg(company);
  const deliveryAddress = buildAddress({
    name: pickup.customerName,
    street: pickup.address,
  ...
  const parcel = {
    ...
    ClientReference: String(pickup.ticketId),
    Count: 1,
    Content: `Colet la schimb (livrare) — ${pickup.ticketId}`,
    PickupDate: toGlsDate(new Date()),
    PickupAddress: pickupAddress,
    DeliveryAddress: deliveryAddress,
  };
...
module.exports = { isConfigured, createParcel, createPickupAwb, createExchangeAwb, deleteParcel, getParcelStatus, getLabelPdf, findByClientReference, getBaseUrl };
