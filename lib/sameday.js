...
async function request(company, method, path, { body, query } = {}, retryOn401 = true) {
  ...
  if (!res.ok) {
    ...
    const err = new Error(`Sameday: eroare ${res.status} la ${method} ${path} — ${detail}`);
 ...
 */
/**
 * Istoricul complet de status, pentru un AWB (expeditie) -- CONFIRMAT LIVE
 * cu contul real: GET /api/client/awb/{awbNumber}/status. Folosit numarul
 * de baza (fara sufix de colet) -- acest endpoint acopera automat toate
 * coletele expedierii (campul "expeditionHistory"), indiferent daca sunt
 * unul sau mai multe, intr-un istoric combinat, cronologic. Gasit prin
 * campul "expeditionDetails" din raspunsul status-sync -- contrazice
 * raspunsul initial al suportului tehnic Sameday (care spusese ca nu
 * exista un astfel de endpoint).
 */
async function getAwbStatus(company, awbNumber) {
  const data = await request(company, 'GET', `/api/client/awb/${awbNumber}/status`);
