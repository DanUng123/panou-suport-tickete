...
function isConfigured() {
  const c = cfg();
  return Boolean(c.username && c.password && c.pickupPointAddress);
}
...
async function request(method, path, { body, query } = {}, retryOn401 = true) {
  ...
  if (!res.ok) {
    throw new Error(`Sameday: eroare ${res.status} la ${method} ${path} — ${data.message || JSON.stringify(data).slice(0, 300)}`);
  }
  return data;
}
...
async function createPickupAwb(pickup) {
  ...
  const serviceCode = pickup.reason === 'schimb' ? 'CS' : 'RS';
  ...
  const body = {
    ...
    thirdParty: party,
    awbRecipient: party, // in sensul invers: destinatarul "oficial" e tot clientul (ridicam de la el, catre noi)
    clientInternalReference: String(pickup.ticketId),
    parcels: [{ weight: 1 }],
  };
