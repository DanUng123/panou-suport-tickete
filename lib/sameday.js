...
async function getAwbPdf(company, awbNumber) {
  let token = tokenCache.get(company.id);
  if (!token) token = await authenticate(company);
  // format A4/A6, ales de companie -- numele exact al parametrului HTTP
  // brut NU a fost confirmat live cu API-ul real Sameday (doar conceptul
  // A4/A6 e confirmat, din documentatia lor publica pentru alte SDK-uri);
  // trimis DOAR daca a fost ales explicit, ca sa nu schimbam comportamentul
  // companiilor care nu ating deloc aceasta setare noua -- de verificat
  // impreuna, la testarea live
  const format = company.samedayAwbPdfFormat === 'A4' || company.samedayAwbPdfFormat === 'A6' ? company.samedayAwbPdfFormat : null;
  const url = `${getBaseUrl(company)}/api/awb/download/${awbNumber}${format ? `?type=${format}` : ''}`;
  const res = await fetch(url, {
