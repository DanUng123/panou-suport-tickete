...
/**
 * Rezolva ID-ul persoanei de contact de la punctul de ridicare (camp nou,
 * confirmat prin documentatia oficiala -- optional, dar recomandat).
 * Prioritate: SAMEDAY_CONTACT_PERSON_ID explicit > persoana implicita
 * (defaultContactPerson) de la punctul de ridicare folosit.
 */
async function resolveContactPersonId() {
  const explicitId = process.env.SAMEDAY_CONTACT_PERSON_ID;
  if (explicitId) return Number(explicitId);
  try {
    const pickupId = await resolvePickupPointId();
    const points = await getPickupPoints();
...
async function createPickupAwb(pickup) {
  ...
  const contactPersonId = await resolveContactPersonId();

  const services = await getServices();
  // coduri confirmate printr-un apel real catre cont: RS = "Retur Standard"
  // (ridicare de la client -- folosit si pentru "service"), "24" = livrare
  // standard (folosit pentru "schimb", vezi mai jos de ce).
  ...
  if (!c.senderName || !c.senderPhone || !c.senderPostalCode || !c.senderAddress) {
    throw new Error('Sameday: datele voastre de contact (SAMEDAY_SENDER_NAME/PHONE/POSTAL_CODE/ADDRESS) nu sunt configurate pe server.');
  }
 ...
 * Interogam ultimele 2 ore si filtram raspunsul dupa numarul AWB-ului cautat.
 * LIMITARE CUNOSCUTA: daca AWB-ul are evenimente mai vechi de ~2 ore, e
 * posibil sa nu apara cu aceasta metoda -- de reverificat la primul test real
 * (structura exacta a campurilor din raspuns e de asemenea neconfirmata,
 * doc-ul nu da un exemplu JSON pentru acest endpoint).
 */
async function getAwbStatus(awbNumber) {
  const now = Date.now();
  const twoHoursAgo = now - 2 * 60 * 60 * 1000;
  const data = await request('GET', '/api/client/status-sync', {
    query: {
  ...
  const matching = allEvents.filter((e) => {
    const evAwb = String(e.awbNumber || e.awb || '');
    return evAwb === String(awbNumber) || evAwb.startsWith(String(awbNumber));
  });
