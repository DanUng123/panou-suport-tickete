...
// clientii ei. Asa, importul e o singura data pe zi, in fundal, iar cautarea
// e locala.
...
const REIMPROSPATARE_MS = Number(process.env.CATALOG_REFRESH_HOURS || 24) * 3600 * 1000;
...
async function importCatalogForCompany(company) {
  ...
  try {
    ...
    while (start < MAX_PRODUSE) {
      ...
      if (!produse.length) break;
      total += db.upsertProducts(company.id, produse, eticheta);
