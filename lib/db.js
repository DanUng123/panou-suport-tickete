...
function rowToOrder(row) {
  ...
  try {
    ...
    // GoMag trimite mereu un obiect "invoice" chiar si cand nu exista o
    // factura reala emisa (ex: {series:"", number:"0"}) -- tratam un numar
    // lipsa/zero ca "fara factura", exact ca la MerchantPro
    if (invoice && (!invoice.number || invoice.number === '0')) invoice = null;
...
function listOrders(companyId, filters = {}) {
  ...
  let sql = `SELECT * FROM orders ${clauses.length ? 'WHERE ' + clauses.join(' AND ') : ''} ORDER BY dateCreated DESC`;
  if (filters.limit) {
    sql += ' LIMIT ?';
