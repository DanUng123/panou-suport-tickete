...
const ALLOWED_SECTIONS = ['support', 'service', 'retur'];
 ...
 */
function setTicketPickupAwb(ticketId, { awbNumber, parcelId, labelPdf, section }, actingAgent) {
  ...
  db.prepare(`
    UPDATE tickets SET pickupAwbNumber = ?, pickupAwbParcelId = ?, pickupAwbLabelPdf = ?, pickupAwbCreatedAt = ?, section = ?, updatedAt = ?
    WHERE id = ?
  `).run(awbNumber, String(parcelId), labelPdf || null, nowISO(), section, nowISO(), ticketId);
...
function clearTicketPickupAwb(ticketId, actingAgent) {
  ...
  if (!existing) return null;
  db.prepare(`
    UPDATE tickets SET pickupAwbNumber = NULL, pickupAwbParcelId = NULL, pickupAwbLabelPdf = NULL, pickupAwbCreatedAt = NULL, updatedAt = ?
    WHERE id = ?
  `).run(nowISO(), ticketId);
  return getTicket(ticketId);
}
...
function getOrderStats(filters = {}) {
  ...
  function withDate(extraConds) {
    const all = [...extraConds, ...dateConds];
    return all.length ? `WHERE ${all.join(' AND ')}` : '';
  }

  const total = db.prepare(`SELECT COUNT(*) AS c FROM orders ${withDate([])}`).get(...dateParams).c;
  const needsAwb = db.prepare(`SELECT COUNT(*) AS c FROM orders ${withDate([
    "(awbNumber IS NULL OR awbNumber = '')",
    "(shippingAwb IS NULL OR shippingAwb = '')",
    "shippingStatus NOT IN ('cancelled', 'delivered', 'returned')",
  ])}`).get(...dateParams).c;
  const withAwb = db.prepare(`SELECT COUNT(*) AS c FROM orders ${withDate([
    "((awbNumber IS NOT NULL AND awbNumber != '') OR (shippingAwb IS NOT NULL AND shippingAwb != ''))",
  ])}`).get(...dateParams).c;

  const byShippingStatus = {};
  db.prepare(`SELECT shippingStatus, COUNT(*) AS c FROM orders ${withDate([])} GROUP BY shippingStatus`).all(...dateParams)
    .forEach((r) => { byShippingStatus[r.shippingStatus || 'necunoscut'] = r.c; });

  const byPaymentStatus = {};
  db.prepare(`SELECT paymentStatus, COUNT(*) AS c FROM orders ${withDate([])} GROUP BY paymentStatus`).all(...dateParams)
  ...
  return {
    total, needsAwb, withAwb,
    shipped: byShippingStatus.shipped || 0,
    cancelled: byShippingStatus.cancelled || 0,
