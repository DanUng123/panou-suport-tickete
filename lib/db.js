...
function clearTicketPickupAwb(ticketId, actingAgent) {
  ...
  if (!existing) return null;
  db.prepare(`
    UPDATE tickets SET pickupAwbNumber = NULL, pickupAwbParcelId = NULL, pickupAwbLabelPdf = NULL, pickupAwbCreatedAt = NULL, pickupAwbCourier = NULL, stage = NULL, updatedAt = ?
    WHERE id = ?
  `).run(nowISO(), ticketId);
  return getTicket(ticketId);
}
