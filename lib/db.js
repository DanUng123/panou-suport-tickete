...
function upsertOrderFromGomag(companyId, gm) {
  const id = `ORD_${companyId}_${gm.id}`;
  const existing = db.prepare('SELECT id, internalStatus, assignedTo, awbCourier, awbNumber, awbCreatedAt FROM orders WHERE mpId = ? AND companyId = ?').get(gm.id, companyId);
