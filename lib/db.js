...
/** Sterge TOATE tichetele DINTR-O COMPANIE (si, prin cascada, comentarii/jurnal/fotografii asociate). Comenzile NU sunt afectate. */
// ---------- clienti importati (Excel) ----------

/** Lista completa a clientilor salvati, pentru o companie. */
function listClients(companyId) {
  return db.prepare('SELECT * FROM clients WHERE companyId = ? ORDER BY createdAt DESC').all(companyId)
 ...
 * telefon, in aceeasi companie).
 */
function importClients(companyId, rows) {
  const existing = new Set(
    db.prepare('SELECT phoneNormalized FROM clients WHERE companyId = ?').all(companyId).map((r) => r.phoneNormalized)
  );

  const ins = db.prepare(`
    INSERT INTO clients (id, companyId, name, phone, phoneNormalized, email, address, city, extraJson, createdAt)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  for (const row of rows) {
    ...
    if (!normalized) { noPhone += 1; continue; }
    if (existing.has(normalized) || seenInFile.has(normalized)) { duplicates += 1; continue; }
    seenInFile.add(normalized);
    ins.run(
      genId('CLI'), companyId, row.name || null, row.phone || null, normalized,
      row.email || null, row.address || null, row.city || null,
      row.extra ? JSON.stringify(row.extra) : null, nowISO()
    );
    saved += 1;
  }

  return { totalRows: rows.length, saved, duplicates, noPhone };
}
