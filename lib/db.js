 ...
 */
const SECTION_CODE_PREFIX = { service: 'S', retur: 'R' };
...
function setTicketPickupAwb(ticketId, { awbNumber, parcelId, labelPdf, section, pickupAddress, pickupCity, pickupPostalCode, pickupPhone }, actingAgent) {
  ...
  const stage = existing.stage || 'in_transit_to_service';

  db.prepare(`
    UPDATE tickets SET pickupAwbNumber = ?, pickupAwbParcelId = ?, pickupAwbLabelPdf = ?, pickupAwbCreatedAt = ?,
    ...
    WHERE id = ?
  `).run(
    awbNumber, String(parcelId), labelPdf || null, existing.pickupAwbCreatedAt || nowISO(), section, sectionCode, stage,
...
function clearTicketPickupAwb(ticketId, actingAgent) {
  ...
  if (!existing) return null;
  db.prepare(`
    UPDATE tickets SET pickupAwbNumber = NULL, pickupAwbParcelId = NULL, pickupAwbLabelPdf = NULL, pickupAwbCreatedAt = NULL, stage = NULL, updatedAt = ?
    WHERE id = ?
  `).run(nowISO(), ticketId);
  return getTicket(ticketId);
}
...
function setTicketReturnAwb(ticketId, { awbNumber, parcelId, labelPdf }, actingAgent) {
  const existing = db.prepare('SELECT * FROM tickets WHERE id = ?').get(ticketId);
  if (!existing) return null;
  db.prepare(`
    UPDATE tickets SET returnAwbNumber = ?, returnAwbParcelId = ?, returnAwbLabelPdf = ?, returnAwbCreatedAt = ?,
      stage = 'in_transit_to_client', updatedAt = ?
    WHERE id = ?
  `).run(awbNumber, String(parcelId), labelPdf || null, nowISO(), nowISO(), ticketId);
  logChange(ticketId, actingAgent, 'stage', existing.stage, 'in_transit_to_client');
  return getTicket(ticketId);
}
...
function clearTicketReturnAwb(ticketId, actingAgent) {
  ...
  if (!existing) return null;
  db.prepare(`
    UPDATE tickets SET returnAwbNumber = NULL, returnAwbParcelId = NULL, returnAwbLabelPdf = NULL, returnAwbCreatedAt = NULL,
