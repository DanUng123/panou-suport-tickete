...
function getAllPlatformClients({ page = 1, pageSize = 100, q = '', companyId = null } = {}) {
  ...
  const rows = db.prepare(`
    ...
    FROM orders o
    JOIN companies comp ON comp.id = o.companyId
    ${whereSql}
...
function listAllCompaniesForAdmin() {
  const rows = db.prepare('SELECT id, name, createdAt, active FROM companies ORDER BY createdAt DESC').all();
...
/** Câte rânduri deține compania, pe tabele -- pentru confirmarea din interfață. */
function getCompanyDataCounts(companyId) {
  const one = (sql) => db.prepare(sql).get(companyId)?.n || 0;
  return {
    orders: one('SELECT COUNT(*) AS n FROM orders WHERE companyId = ?'),
    tickets: one('SELECT COUNT(*) AS n FROM tickets WHERE companyId = ?'),
    comments: one('SELECT COUNT(*) AS n FROM comments WHERE companyId = ?'),
    orderNotes: one('SELECT COUNT(*) AS n FROM order_notes WHERE companyId = ?'),
    ticketPhotos: one('SELECT COUNT(*) AS n FROM ticket_photos WHERE companyId = ?'),
    auditLog: one('SELECT COUNT(*) AS n FROM audit_log WHERE companyId = ?'),
    trackingEvents: one('SELECT COUNT(*) AS n FROM sameday_tracking_events WHERE companyId = ?'),
    categories: one('SELECT COUNT(*) AS n FROM categories WHERE companyId = ?'),
    agents: one('SELECT COUNT(*) AS n FROM agents WHERE companyId = ?'),
  };
}
module.exports.getCompanyDataCounts = getCompanyDataCounts;

/**
 * Șterge DEFINITIV o companie și tot ce ține de ea. Ireversibil.
 * Întoarce numărul de rânduri șterse, pe tabelă.
 */
function deleteCompanyCompletely(companyId) {
  ...
  return runInTransaction(() => {
    const deleted = {};
    const run = (key, sql) => {
      const info = db.prepare(sql).run(companyId);
      deleted[key] = Number(info.changes || 0);
    };

    // 1. indexul de căutare -- înaintea comenzilor, cât încă le putem găsi rowid-ul
    const fts = db.prepare(
      'DELETE FROM orders_fts WHERE rowid IN (SELECT rowid FROM orders WHERE companyId = ?)'
    ).run(companyId);
    deleted.ordersFts = Number(fts.changes || 0);

    // 2. tabele-copil ale tichetelor
    run('comments', 'DELETE FROM comments WHERE companyId = ?');
    run('auditLog', 'DELETE FROM audit_log WHERE companyId = ?');
    run('ticketPhotos', 'DELETE FROM ticket_photos WHERE companyId = ?');

    // 3. tichetele (eliberează referințele către comenzi) și abia apoi comenzile
    run('tickets', 'DELETE FROM tickets WHERE companyId = ?');
    run('orderNotes', 'DELETE FROM order_notes WHERE companyId = ?');
    run('orders', 'DELETE FROM orders WHERE companyId = ?');

    // 4. restul datelor companiei
    run('trackingEvents', 'DELETE FROM sameday_tracking_events WHERE companyId = ?');
    run('sectionCounters', 'DELETE FROM section_counters WHERE companyId = ?');
    run('categories', 'DELETE FROM categories WHERE companyId = ?');

    // 5. conturile de utilizator (jetoanele de resetare cad prin cascadă pe agents)
    const tokens = db.prepare(
      'DELETE FROM password_reset_tokens WHERE agentId IN (SELECT id FROM agents WHERE companyId = ?)'
    ).run(companyId);
    deleted.passwordResetTokens = Number(tokens.changes || 0);
    run('agents', 'DELETE FROM agents WHERE companyId = ?');

    // 6. compania însăși -- cu tot cu credențialele de integrare (MerchantPro,
    // GoMag, GLS, Sameday, PTT), care sunt coloane pe rândul acesta
    const comp = db.prepare('DELETE FROM companies WHERE id = ?').run(companyId);
    deleted.company = Number(comp.changes || 0);

    return { companyName: company.name, deleted };
  });
}
