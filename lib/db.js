 ...
 * -- folosita EXCLUSIV de panoul de administrare al platformei (creatorul
 * platformei, nu un manager de companie). Deduplicare globala dupa telefon
 * normalizat -- pastram datele din cea mai recenta comanda gasita pentru
 * fiecare numar unic (comenzile sunt parcurse in ordine descrescatoare
 * dupa data crearii, deci primul telefon intalnit e cel mai recent).
 */
function getAllPlatformClients({ page = 1, pageSize = 100, q = '' } = {}) {
  const rows = db.prepare(`
    SELECT shippingName, billingName, customerEmail, shippingPhone, shippingAddress, shippingCity, shippingState
    FROM orders
    WHERE shippingPhone IS NOT NULL AND shippingPhone != ''
    ORDER BY dateCreated DESC
  `).all();

  const seen = new Map();
  for (const row of rows) {
    const norm = normalizePhone(row.shippingPhone);
    if (!norm || seen.has(norm)) continue;
    seen.set(norm, {
      name: row.shippingName || row.billingName || null,
      phone: row.shippingPhone,
      email: row.customerEmail || null,
      address: row.shippingAddress || null,
      city: row.shippingCity || null,
      county: row.shippingState || null,
    });
  }
  ...
  const items = clients.slice(offset, offset + pageSize);
