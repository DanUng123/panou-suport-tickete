...
function listOrders(filters = {}) {
  ...
  if (filters.hasAwb) {
    clauses.push("(awbNumber IS NOT NULL AND awbNumber != '')");
  }
  if (filters.q) {
...
function getOrderStats() {
  ...
  const withAwb = db.prepare("SELECT COUNT(*) AS c FROM orders WHERE awbNumber IS NOT NULL AND awbNumber != ''").get().c;
  ...
  db.prepare('SELECT internalStatus, COUNT(*) AS c FROM orders GROUP BY internalStatus').all()
    .forEach((r) => { byInternalStatus[r.internalStatus] = r.c; });

  const byAssignedTo = {};
  db.prepare('SELECT assignedTo, COUNT(*) AS c FROM orders GROUP BY assignedTo').all()
    .forEach((r) => { byAssignedTo[r.assignedTo || 'unassigned'] = r.c; });

  const lastSync = db.prepare('SELECT MAX(syncedAt) AS t FROM orders').get().t;
  return { total, needsAwb, withAwb, byShippingStatus, byPaymentStatus, byInternalStatus, byAssignedTo, lastSync };
}
