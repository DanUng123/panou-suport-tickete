...
function isConfigured(company) {
  return Boolean(company.gomagShopUrl && company.gomagApiKey && company.gomagActive !== false);
}
...
async function request(company, method, path, params = {}) {
  ...
  // GoMag compara `ApiShop` cu adresa magazinului exact cum e trecuta in
  // panoul lui de administrare. O spatiere in plus sau un `/` la final o fac
  // sa nu mai coincida, iar raspunsul e un refuz sec, greu de explicat --
  // asa ca o curatam aici.
  const apiShop = String(company.gomagShopUrl || '').trim().replace(/\/+$/, '');
  ...
  } catch (e) { /* raspuns care nu e JSON -- pastram textul brut */ }
  if (!res.ok) {
    // mesajul trebuie sa spuna ce s-a intamplat, nu doar ca "n-a mers":
    // codul HTTP, ce a raspuns GoMag si cu ce adresa de magazin am cerut
    const detaliu = (data && (data.message || data.error)) || (text || '').replace(/\s+/g, ' ').trim().slice(0, 200);
    const explicatie = res.status === 401 || res.status === 403
      ? 'cheie API respinsă sau acces API neactivat pentru magazin'
      : res.status === 404 ? 'adresă API inexistentă' : '';
    throw new Error(`GoMag a răspuns ${res.status}${explicatie ? ` (${explicatie})` : ''}${detaliu ? `: ${detaliu}` : ''} — magazin trimis: ${apiShop || '(gol)'}`);
  }
  if (data && data.error) {
    // GoMag intoarce uneori 200 OK, dar cu eroare in corpul raspunsului
    // (ex: "Nu exista cheia API") -- verificam explicit si acest caz
    throw new Error(`${data.message || `Eroare GoMag ${data.error}`} — magazin trimis: ${apiShop || '(gol)'}`);
  }
  return data;
}
