...
/**
 * Lista serviciilor disponibile pe contul companiei (ID + nume + tarif) --
 * CONFIRMAT LIVE cu credentiale reale. Lista difera de la cont la cont, deci
 * o citim de la ei, nu o presupunem. Serviciile se calculeaza pentru o ruta
 * concreta, asa ca trimitem o expediere-etalon: de la adresa expeditorului
 * companiei catre un oras din alt judet (ca sa apara si serviciile nationale,
 * nu doar cele locale), cu un colet standard de 1 kg.
 * ATENTIE: e doar o interogare de tarifare -- NU creeaza nicio expediere.
 */
async function getAvailableServices(company) {
  const c = company;
  const readyDate = new Date().toISOString().slice(0, 19);
  const shipFrom = locationXml('ShipFrom', {
    name: c.pttSenderName || c.companyName || 'Expeditor',
    address: c.pttSenderAddress || 'Strada Exemplu 1',
    city: c.pttSenderCity || 'Bucuresti',
    postCode: c.pttSenderPostalCode || '010101',
    contact: c.pttSenderPhone || '0700000000',
    email: c.pttSenderEmail || '',
    isPrivatePerson: false,
  });
  // destinatie-etalon, intr-un alt judet decat majoritatea expeditorilor, ca
  // lista sa contina si serviciile nationale
  const shipTo = locationXml('ShipTo', {
    name: 'Destinatar', address: 'Strada Exemplu 2', city: 'Cluj-Napoca',
    postCode: '400001', contact: '0700000000', email: c.pttSenderEmail || '', isPrivatePerson: true,
  });

  const bodyXml = `<GetAvailableServices xmlns="${SOAP_NS}">${authTokenXml(company)}
  ...
  assertSuccess(xml, 'GetAvailableServices');

  return extractAllBlocks(xml, 'Service')
    .map((block) => ({
