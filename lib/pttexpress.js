...
async function createPickupAwb(company, pickup) {
  ...
  const shipFrom = locationXml('ShipFrom', {
    ...
    postCode: pickup.postalCode, contact: pickup.phone, email: pickup.email, isPrivatePerson: true,
  });
...
async function createForwardAwb(company, order) {
  ...
  const shipTo = locationXml('ShipTo', {
    ...
    postCode: order.shippingPostalCode, contact: order.shippingPhone, email: order.customerEmail, isPrivatePerson: true,
  });
  const bodyXml = `<CreateShipment xmlns="${SOAP_NS}">
    ${authTokenXml(company)}
    <shipmentRequest>
...
async function getLabelPdf(company, awbNumber) {
  const bodyXml = `<GetLabel xmlns="${SOAP_NS}">${authTokenXml(company)}<getLabelRequest><PackageNo><string>${xmlEscape(awbNumber)}</string></PackageNo><LabelFormat>PDFA4</LabelFormat></getLabelRequest></GetLabel>`;
